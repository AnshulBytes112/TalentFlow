...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const endpoint = role === 'recruiter' ? '/api/analytics/recruiter' : '/api/application/my';
      ...
      <div className="space-y-10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-text-secondary font-medium tracking-tight">
              {role === 'recruiter' 
                ...
                : "Analyze your applications and explore elite opportunities today."}
            </p>
          </div>
        ...
        >
          {role === 'recruiter' ? (
            <>
              {[
                ...
                <motion.div key={idx} variants={item}>
                  <Card className="hover:border-accent-primary/30 transition-colors cursor-pointer group">
            ...
            <>
              {[
                { label: 'Applied Jobs', value: stats?.length || '18', icon: Briefcase, color: 'text-accent-primary', bg: 'bg-accent-primary/10' },
                { label: 'Interviews', value: '4', icon: Users, color: 'text-accent-secondary', bg: 'bg-accent-secondary/10' },
                { label: 'Profile Views', value: '1,240', icon: TrendingUp, color: 'text-luxury', bg: 'bg-luxury/10' },
                { label: 'Response Rate', value: '92%', icon: CheckCircle2, color: 'text-accent-warning', bg: 'bg-accent-warning/10' },
              ].map((stat, idx) => (
                <motion.div key={idx} variants={item}>
                  <Card className="hover:border-accent-primary/30 transition-colors cursor-pointer group">
                    <CardContent className="pt-6">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
           <Card className="lg:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-xl uppercase tracking-tight">Recent Activity</CardTitle>
                <button className="text-xs font-bold text-accent-primary hover:underline">View All</button>
              </CardHeader>
              <CardContent className="space-y-6">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-elevated/20 border border-border/50 hover:bg-elevated/40 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-text-primary">
                          {role === 'recruiter' ? 'New Application Received' : 'Application Status Updated'}
                        </h4>
                        <p className="text-xs text-text-tertiary">Senior Full Stack Engineer • Just now</p>
                      </div>
                    </div>
                    <Badge variant={i % 2 === 0 ? 'offer' : 'applied'}>
           ...
           <Card className="bg-gradient-to-br from-bg-card to-accent-primary/5">
              <CardHeader>
                <CardTitle className="text-xl uppercase tracking-tight">Elite Performance</CardTitle>
              </CardHeader>
                ...
                <div className="flex flex-col items-center justify-center p-8 text-center space-y-4">
                   ...
                   <p className="text-sm text-text-secondary font-medium px-4">
                      {role === 'recruiter' 
                        ...
                        : "Your profile visibility has increased by 12%."}
                   </p>
                </div>
                <button className="w-full py-4 text-xs font-black uppercase tracking-[0.2em] bg-bg-primary border border-border hover:border-accent-primary transition-all rounded-xl">
